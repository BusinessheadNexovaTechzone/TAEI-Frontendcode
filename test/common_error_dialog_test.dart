...
void main() {
  group('CommonErrorDialog backend error extraction', () {
    ...
    test('maps mobile, OTP, ABHA, address, transaction, face, fingerprint, network, server and unknown errors', () {
      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'Invalid Mobile Number'}),
      ...
      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'OTP validation failed'}),
        'The OTP you entered is incorrect or has expired.\n\nPlease check the OTP and try again, or request a new OTP.',
      );

      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'Invalid ABHA Number'}),
        'The ABHA number you entered is invalid.\n\nPlease check your 14-digit ABHA number and try again.',
      );

      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'ABHA Address already exists'}),
        'This ABHA Address is already in use.\n\nPlease choose another unique ABHA Address.',
      );

      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'Invalid Transaction Id'}),
        'Your session has expired.\n\nPlease start the verification process again.',
      );

      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'Face Verification Failed'}),
        'Face verification could not be completed.\n\nPlease scan your face again and ensure your face is clearly visible.',
      );

      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'Fingerprint Capture Failed'}),
        'Fingerprint capture was unsuccessful.\n\nPlease place your finger properly on the scanner and try again.',
      );

      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'SocketException'}),
        'Unable to connect to the server.\n\nPlease check your internet connection and try again.',
      );

      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'Internal Server Error'}),
        'Something went wrong on the server.\n\nPlease try again after a few moments.',
      );

      expect(
        CommonErrorDialog.extractFriendlyErrorMessage({'message': 'This is completely unexpected'}),
