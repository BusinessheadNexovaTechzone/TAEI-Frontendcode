...
class PatientReportScreen extends StatefulWidget {
  ...
  PatientReportScreen({required this.hospitalid});
  @override
...
class _PatientReportScreenState extends State<PatientReportScreen> {
  ...
  fetchData() async {
    controller.patientHospitalId.value = widget.hospitalid;
            ...
            return SingleChildScrollView(
              child: Padding(
                ...
                child: Column(
                  children: [
                    PillarChipSelector(
                      selectedPillar: controller.patientPillerType,
                      onChanged: (value) async {
                        controller.patientPillerType.value = value;
                        await controller.getPatientReport();
                      },
                    ),
                    Space(height: 20),
                    NewTitleDropdown(
                        ...
                        items: controller.entryData.value!.data!
                            .map((e) =>
                                {"id": e.hospitalid, "name": e.hospitalName})
                            .toList(),
                        selectedId: controller.patientHospitalId.value,
                        onChanged: (value) async {
                          controller.patientHospitalId.value = value!;
                          await controller.getPatientReport();
                        }),
                    Space(height: 20),
                    NewTitleDropdown(
