...
TriageEdLineListModel triageEdLineListModelFromJson(String str) =>
    TriageEdLineListModel.fromJson(json.decode(str));

String triageEdLineListModelToJson(TriageEdLineListModel data) =>
    json.encode(data.toJson());

class TriageEdLineListModel {
  List<TriageEdLIneListDate>? cases;
  int? pageCount;

  TriageEdLineListModel({
    this.cases,
    this.pageCount,
  });

  factory TriageEdLineListModel.fromJson(Map<String, dynamic> json) {
    return TriageEdLineListModel(
      cases: json["data"] == null || json["data"].isEmpty
          ? []
          : List<TriageEdLIneListDate>.from(
              json["data"][0].map((x) => TriageEdLIneListDate.fromJson(x))),
  ...
  Map<String, dynamic> toJson() => {
        "data": [
          cases?.map((x) => x.toJson()).toList(),
        ],
        "Page_Count": pageCount,
      };
}

class TriageEdLIneListDate {
  int? rowNum;
  dynamic patientName;
  int? caseId;
  ...
  factory TriageEdLIneListDate.fromJson(Map<String, dynamic> json) =>
      TriageEdLIneListDate(
        rowNum: json["row_num"],
